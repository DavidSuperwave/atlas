...
export default function ScrapeDetailsPage() {
    ...
    const [cancellingEnrichment, setCancellingEnrichment] = useState(false);
    ...
    // Cancel enrichment for this scrape
    async function handleCancelEnrichment() {
        if (!confirm('Are you sure you want to cancel enrichment? Leads currently being processed will be reset.')) {
            return;
        }
        
        setCancellingEnrichment(true);
        try {
            const res = await apiFetch(`/api/enrich/${id}/cancel`, {
                method: 'POST',
            });
            const data = await res.json();
            if (data.success) {
                alert(`Enrichment cancelled. ${data.leadsReset} leads reset, ${data.queuedRemoved} removed from queue.`);
                fetchData();
            } else {
                alert('Failed to cancel enrichment: ' + (data.error || 'Unknown error'));
            }
        } catch (err) {
            alert('Error cancelling enrichment');
            console.error(err);
        } finally {
            setCancellingEnrichment(false);
        }
    }
                ...
                <div className="max-w-7xl mx-auto px-6 py-4">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {/* Cancel Enrichment Button - shows when processing */}
                            {processingCount > 0 && (
                                <button
                                    onClick={handleCancelEnrichment}
                                    disabled={cancellingEnrichment}
                                    className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium text-sm transition-all shadow-sm ${
                                        cancellingEnrichment
